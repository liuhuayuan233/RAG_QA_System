...
class Retriever:
    ...
    def retrieve_by_keywords(self, keywords: List[str], k: int = None) -> List[Dict[str, Any]]:
        """基于关键词检索"""
        try:
            if not keywords:
                return []
    ...
    def retrieve_by_document(self, filename: str, k: int = None) -> List[Dict[str, Any]]:
        """检索特定文档的内容"""
        try:
            if k is None:
                k = self.config.TOP_K_RETRIEVAL
            ...
            logger.info(f"检索文档 {filename}，找到 {len(results)} 个块")
            return results
    ...
    def multi_query_retrieve(self, queries: List[str], k: int = None) -> List[Dict[str, Any]]:
        """多查询检索"""
        try:
            if not queries:
                return []
            ...
            # 重新排序
            all_results.sort(key=lambda x: x.get("final_score", x["score"]), reverse=True)
            
            # 限制返回数量
            if k:
                all_results = all_results[:k]
            
            logger.info(f"多查询检索完成，合并后返回 {len(all_results)} 个结果")
            return all_results
            
        except Exception as e:
            logger.error(f"多查询检索失败: {str(e)}")
            return []
